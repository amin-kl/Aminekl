...
CONF = {
    ...
    "OWNER_ID":      int(os.getenv("BOT2_OWNER", "8901983423")),
    "CONTAINER_URL": os.getenv("CONTAINER_URL",  "docker.io/aminekl2007/vless:latest"),   # يمكن تركه فارغاً → يطلبه من المستخدم
}
...
async def _run_cloud_run(
    ...
    async with async_playwright() as pw:
        ...
        try:
            # ══════════════════════════════════════════════════
            # ① فتح الرابط الطويل SSO — وانتظار اكتمال الـ redirect
            # ══════════════════════════════════════════════════
            await _edit("جاري فتح رابط الدخول...")
            log.info(f"① فتح SSO: {raw_url[:80]}")

            # فتح الرابط بدون timeout صارم — نترك كل الـ redirects تكتمل
            try:
                await page.goto(raw_url, wait_until="networkidle", timeout=60_000)
            except Exception:
                try:
            ...
            # انتظر إضافي حتى تستقر الصفحة
            await asyncio.sleep(4)
            await _snap(page, bot, f"① بعد فتح SSO — {page.url[:70]}")

            # ══════════════════════════════════════════════════
            # ② معالجة كل الشاشات التي تظهر بعد SSO
            # (Welcome / I understand / Sign in عادي / Console)
            # ══════════════════════════════════════════════════
            for _step in range(15):
                _url = page.url.lower()
                try:
                    _html = (await page.content()).lower()
                except Exception:
                    _html = ""

                log.info(f"  [step {_step}] URL: {page.url[:80]}")

                # ✅ وصلنا Console → نكمل
                if "console.cloud.google.com" in _url:
                    await _edit("✅ دخلنا Console بنجاح!")
                    break

                # صفحة "Welcome to your new account" → I understand
                if "i understand" in _html or "welcome to your new account" in _html:
                ...
                # صفحة Sign in عادية → مشكلة في الرابط
                if "accounts.google.com/v3/signin/identifier" in _url:
                    await _snap(page, bot, "⚠️ صفحة Sign in — الرابط لم يُكمل SSO")
                    # لا نتوقف — ننتظر redirect تلقائي
                    await asyncio.sleep(3)
                    continue

                # speedbump / speedb → انتظر redirect
                if "speedb" in _url or "speedbump" in _url:
                    await asyncio.sleep(3)
                    continue

                # Agree and continue / Terms
                if "agree" in _html or "terms of service" in _html:
                    try:
            ...
            # تحقق نهائي
            if "console.cloud.google.com" not in page.url.lower():
                await _snap(page, bot, f"❌ فشل الدخول — الصفحة: {page.url[:80]}")
                return None
